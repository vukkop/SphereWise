...
const HowWeWork = ({ isLight }) => {
  ...
  const adjustStepClasses = (index) => {
    ...
    for (let i = 0; i <= index; i++) {
      isLight
        ? document.getElementById(i.toString()).classList.add('bg-sky-600')
  ...
  const resetStepClasses = (index) => {
    for (let i = 0; i <= index; i++) {
      isLight
        ? document.getElementById(i.toString()).classList.remove('bg-sky-600')
  ...
  return (
    <div className={`row bg-neutral my-10 p-8 ${isLight && 'bg-slate-300'}`}>
